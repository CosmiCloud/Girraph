const db = require('better-sqlite3')(`${__dirname}/../db/pubicdkg.db`,{ verbose: console.log });

module.exports = spam = async (request,ip) => {
  console.log(`Checking if visitor:${ip} is spamming.`)
  //check for spam
  check4ip = await db.prepare('SELECT EXISTS(SELECT 1 FROM request_history WHERE request = ? AND ip_used = ?)').get(request,ip);
  check4ip = JSON.stringify(check4ip);
  check4ip = check4ip[75]

  if(check4ip == 0){//not found
    permission = `allow`
    ...
    await db.prepare('REPLACE INTO request_history VALUES (?,?,?)').run(request,time_stamp,ip);
  }else{
    console.log(`Vistor:${ip} found in request_history.`)
    const spam_result = await db.prepare('SELECT date_last_used FROM request_history WHERE request = ? AND ip_used = ?').get(request,ip);
